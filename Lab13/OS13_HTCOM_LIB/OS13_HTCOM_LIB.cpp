...
HTHANDLE* HT_LIB::HT::Create(OS13HANDEL h, int Capacity, int SecSnapshotInterval, int MaxKeyLength, int MaxPayloadLength, const std::wstring FileName)
{
	try
	{
		HRESULT hr{};
		...
		HTHANDLE *ht{nullptr};
		if ((ht = p_Ht->Create(Capacity, SecSnapshotInterval, MaxKeyLength, MaxPayloadLength, FileName)) == NULL)
...
{
	std::wcout << "(1) FileName: " << ht->FileName << std::endl;
	std::cout << "(2) SnapshotIntervalSec: " << ht->SecSnapshotInterval << std::endl;
	std::cout << "(3) Capacity: " << ht->Capacity << std::endl;
	std::cout << "(4) MaxKeyLength: " << ht->MaxKeyLength << std::endl;
	std::cout << "(5) MaxPayloadLength: " << ht->MaxPayloadLength << std::endl;
}
...
DWORD WINAPI HT_LIB::HT::RecordingWork(LPVOID point)
{
	try {
		...
		std::string str = std::string(typeOperation.begin(), typeOperation.end()) + "\telement:\n\tKey : " + key + "\n\tPayload : " + payload + "\n\tSuccess : " + string_suc + "\n\tOn file : " + std::string(fileName.begin(), fileName.end());
